...
const HTML_TEMPLATE = process.env.QA_REPORT_TEMPLATE || path.join(__dirname, '..', '..', '..', 'QA_테스트_결과서_20260317.html');
const OUTPUT_DIR = process.env.QA_REPORT_OUTPUT_DIR || path.join(process.env.USERPROFILE || process.cwd(), 'Downloads');
const DEPLOY_DIR = process.env.QA_REPORT_DEPLOY_DIR || path.join(OUTPUT_DIR, 'qa-report-deploy');
const VERCEL_SCOPE = process.env.VERCEL_SCOPE || '';

// ── 1. 최신 데이터 가져오기 ──
console.log('[1/3] 스프레드시트에서 데이터 수집 중...');
try {
  execSync(`"${process.execPath}" fetch_qa_status.js`, { cwd: __dirname, stdio: 'inherit' });
} catch (e) {
...
if (tokenArg) {
  if (!VERCEL_SCOPE) {
    console.error('VERCEL_SCOPE 환경변수가 필요합니다.');
    process.exit(1);
  }
