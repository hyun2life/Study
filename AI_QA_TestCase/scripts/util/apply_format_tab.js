...
const DEFAULT_SHEET_ID = process.env.MASTER_SPREADSHEET_ID || '';
...
async function applyFormatToTab(tabName, addConsole = false, spreadsheetId = DEFAULT_SHEET_ID) {
  ...
  // ── 빈 행 정리 (데이터 + 여유 10행만 유지) ────
  const totalRows = sheetMeta.properties.gridProperties.rowCount;
  const keepRows = endRow + 10; // 헤더(1) + 데이터(dataCount) + 여유(10)
  if (totalRows > keepRows) {
    ...
    console.log(`🧹 [${tabName}] 빈 행 ${totalRows - keepRows}개 삭제 (총 ${keepRows}행 유지)`);
  }
}
...
if (require.main === module) {
  ...
  if (isPlaceholder(spreadsheetId)) {
    console.error('스프레드시트 ID가 필요합니다. --spreadsheet 또는 MASTER_SPREADSHEET_ID를 설정하세요.');
    process.exit(1);
  }
