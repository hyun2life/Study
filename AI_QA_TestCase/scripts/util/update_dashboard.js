...
if (!argId && isPlaceholder(process.env.SPREADSHEET_ID) && isPlaceholder(process.env.MASTER_SPREADSHEET_ID)) {
  console.error('사용법: node update_dashboard.js <스프레드시트ID>');
  process.exit(1);
}
const SPREADSHEET_ID = argId || process.env.SPREADSHEET_ID || process.env.MASTER_SPREADSHEET_ID;
if (isPlaceholder(SPREADSHEET_ID)) {
  console.error('유효한 스프레드시트 ID가 필요합니다.');
  process.exit(1);
}
