...
async function getAuthClient() {
    const oauthPath = resolvePath(OAUTH_PATH);
    const tokenPath = resolvePath(TOKEN_PATH);
    ...
    if (fs.existsSync(tokenPath)) {
        ...
        log('✔ 저장된 OAuth 토큰으로 인증 완료');
        return oauth2Client;
    }

    // 최초 인증: 브라우저에서 구글 로그인
    log('🔐 최초 인증이 필요합니다. 브라우저가 열립니다...');
