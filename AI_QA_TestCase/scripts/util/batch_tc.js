...
const CONFIG = path.join(PROJECT_ROOT, 'team', 'batch_config.json');

function resolveClaudeInvocation() {
  if (CLI_JS && fs.existsSync(CLI_JS)) {
    return { command: NODE, argsPrefix: [CLI_JS] };
  }
...
function runPipeline(sheetUrl, confluenceUrl, index, total) {
  ...
  const claude = resolveClaudeInvocation();

  const result = spawnSync(
    claude.command,
    [...claude.argsPrefix, '-p', '--agent', 'tc-팀-v2', '--model', 'sonnet', '--permission-mode', 'bypassPermissions', prompt],
    {
      stdio: 'inherit',
      timeout: 3600000, // 1시간
      encoding: 'utf8',
