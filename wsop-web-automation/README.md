...
- **대시보드 실행 (`Run.bat`)**: 더블클릭하면 백그라운드(콘솔 창 숨김 모드)에서 웹 서버가 기동되며, 브라우저 새 탭에서 **Web UI 테스팅 대시보드**가 자동으로 열립니다. 실행 후 원래의 검은색 cmd 창은 즉시 스스로 종료되어 화면에 불필요한 잔재 창이 남지 않습니다.
- **대시보드 종료 (`Stop.bat`)**: 더블클릭하면 3000번 포트(또는 커스텀 포트)를 점유하고 있는 백그라운드 대시보드 웹 서버 프로세스를 찾아내어 안전하게 종료합니다.
